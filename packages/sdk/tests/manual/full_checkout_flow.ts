...
async function main() {
  ...
  console.log(
    `Prices: ${allProductsWithPrices ? allProductsWithPrices.reduce((sum, p) => sum + p.prices.length, 0) : 0}`,
  )
  console.log(
    `Total Revenue: $${(transactions ? transactions.reduce((sum, t) => sum + t.amount, 0) / 100 : 0).toFixed(2)}`,
  )
  console.log('\nDone!')
}
