...
import {
  ...
  listVersions,
  uploadVersion,
  deployVersion as deployVersionHandler,
  rollbackDeployment,
  HttpError,
...
projects.get("/:id/deployments", zValidator("param", idParam), async (c) => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Failed to list deployments"
    console.error("[deployments] list failed", { projectId: id, error: message })
    return c.json({ error: message }, 500)
  }
})

// GET /projects/:id/deployment-logs - List deployment logs
projects.get("/:id/deployment-logs", zValidator("param", idParam), async (c) => {
  const { id } = c.req.valid("param")
  ...
  try {
    const logs = await DeploymentLogsService.getLogsByProjectId(id)
    return c.json({ logs })
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to list deployment logs"
    console.error("[deployment-logs] list failed", { projectId: id, error: message })
    return c.json({ error: message }, 500)
  }
})

// GET /projects/:id/versions - List versions
projects.get("/:id/versions", zValidator("param", idParam), async (c) => {
  const { id } = c.req.valid("param")
  ...
  try {
    ...
    console.error("[versions] list failed", { projectId: id, error: message })
    return c.json({ error: message }, 500)
  }
})

// POST /projects/:id/versions/upload - Upload version
projects.post(
  "/:id/versions/upload",
  zValidator("param", idParam),
  zValidator(
    "json",
    z.object({
      workerContent: z.string(),
      metadata: z.any().optional(),
    }),
  ),
  async (c) => {
    const { id } = c.req.valid("param")
    const { workerContent, metadata } = c.req.valid("json")
    ...
    try {
      const versionId = await uploadVersion({ projectId: id, workerContent, metadata })
      return c.json({ versionId })
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to upload version"
      console.error("[versions] upload failed", { projectId: id, error: message })
      return c.json({ error: message }, 500)
    }
  },
)

// POST /projects/:id/versions/deploy - Deploy version
projects.post(
  "/:id/versions/deploy",
  zValidator("param", idParam),
  ...
  async (c) => {
    ...
    try {
      await deployVersionHandler({ projectId: id, versionId })
      return c.json({ deployed: true })
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to deploy version"
      console.error("[versions] deploy failed", { projectId: id, error: message })
      return c.json({ error: message }, 500)
    }
  },
)

// POST /projects/:id/rollback - Rollback deployment
projects.post(
  "/:id/rollback",
  zValidator("param", idParam),
  ...
  async (c) => {
    ...
    try {
      await rollbackDeployment({ projectId: id, deploymentId })
      return c.json({ rolledBack: true })
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to rollback deployment"
      console.error("[deployments] rollback failed", { projectId: id, error: message })
      return c.json({ error: message }, 500)
    }
  },
)
