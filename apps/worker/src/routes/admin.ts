...
admin.get("/overview", requireAdmin, async (c) => {
  ...
  const now = new Date();
  const deployedFilter = sql<boolean>`("project"."deployment"->>'status') = 'deployed'`;
  ...
  const allProjects = await (deployed ? projectsQuery.where(deployedFilter) : projectsQuery)
  ...
  return c.json({
    ...
    pagination: {
      ...
      totalProjects: Number((deployed ? deployedProjectsInRange : projectsInRange)?.count ?? 0),
    },
    totals: {
