...
export async function listDeploymentEnvVars(deploymentUrl: string, deployKey: string): Promise<Record<string, string>> {
...
export async function callQuery(
  ...
  const body: { value?: ConvexValue; message?: string; errorMessage?: string; errorData?: ConvexValue } =
    await res.json();
