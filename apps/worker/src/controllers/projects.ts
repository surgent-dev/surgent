...
import * as DeploymentLogsService from "@/services/deployment-logs"
...
import type { Deployment } from "@/apis/deployer/types"
...
export async function deployProject(args: DeployProjectArgs): Promise<void> {
  ...
  console.log("[deploy] start", { projectId, time: new Date().toLocaleString() })
  ...
  try {
    ...
    await DeploymentLogsService.createLog({
      projectId,
      status: "starting",
      message: `Deployment started: ${scriptName}`,
    })
    ...
    if (build.code !== 0) {
      ...
      await DeploymentLogsService.createLog({
        projectId,
        status: "build_failed",
        message: `Build failed: ${String(build.output).slice(0, 500)}`,
      })
    ...
    await DeploymentLogsService.createLog({
      projectId,
      status: "success",
      message: `Deployment successful: ${scriptName}`,
    })

    console.log("[deploy] success", { projectId, scriptName, time: new Date().toLocaleString() })
  } catch (err: any) {
    console.error("[deploy] failed", { projectId, error: err?.message, time: new Date().toLocaleString() })
...
export async function listDeployments(projectId: string): Promise<DeploymentItem[]> {
  const project = await ProjectService.getProjectById(projectId)
  if (!project) throw new Error(`Project ${projectId} not found`)
  if (!project.sandbox?.deployName) throw new Error("Project not deployed")

  const cfApi = new CloudflareAPI(config.cloudflare.accountId!, config.cloudflare.apiToken!)
  const rawDeployments = await cfApi.listDeployments(project.sandbox.deployName)

  const scriptName = project.sandbox.deployName
  const previewUrl = `https://${scriptName}.surgent.site`

  return rawDeployments.map((d: Deployment) => ({
    id: d.id,
...
export async function listVersions(projectId: string): Promise<any[]> {
  const project = await ProjectService.getProjectById(projectId)
  if (!project) throw new Error(`Project ${projectId} not found`)
  if (!project.sandbox?.deployName) throw new Error("Project not deployed")
  ...
  return await cfApi.listVersions(project.sandbox.deployName)
}

export async function uploadVersion(args: {
  projectId: string
  workerContent: string
  metadata?: any
}): Promise<string> {
  const { projectId, workerContent, metadata } = args
  const project = await ProjectService.getProjectById(projectId)
  if (!project) throw new Error(`Project ${projectId} not found`)
  if (!project.sandbox?.deployName) throw new Error("Project not deployed")
  ...
  const result = await cfApi.uploadVersion(project.sandbox.deployName, versionMetadata, workerContent)
  return result.id
}

export async function deployVersion(args: { projectId: string; versionId: string }): Promise<void> {
  ...
  await cfApi.deployVersion(project.sandbox.deployName, versionId)
  await ProjectService.updateDeploymentStatus(projectId, "deployed")
}

export async function rollbackDeployment(args: { projectId: string; deploymentId: string }): Promise<void> {
  ...
  await cfApi.rollbackDeployment(project.sandbox.deployName, deploymentId)
  await ProjectService.updateDeploymentStatus(projectId, "deployed")
}
