...
import { type MarketplaceListing, useMarketplaceListingsQuery } from '@/queries/marketplace'
import type { MarketplaceUser } from './types'

type MarketplaceContentProps = {
  initialListings?: MarketplaceListing[] | null
}

export default function MarketplaceContent({ initialListings = null }: MarketplaceContentProps) {
  const router = useRouter()
  const [user, setUser] = useState<MarketplaceUser | null>(null)
  const listingsQuery = useMarketplaceListingsQuery(60)
  const listings = listingsQuery.data ?? initialListings ?? []
  const isLoading = listingsQuery.isLoading && initialListings == null && !listingsQuery.data
