...
async function createWebsite(prisma: PrismaClient, name: string, domain: string): Promise<string> {
...
async function generateSiteData(
  ...
  const websiteId = await createWebsite(prisma, config.name, config.domain)
