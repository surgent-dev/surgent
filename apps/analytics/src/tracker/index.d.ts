export type TrackedProperties = {
   ...
   *
   * @description extracted from `window.navigator.language`
   * @example 'https://analytics.surgent.app/docs/getting-started'
   */
  referrer: string
   ...
   * Website ID
   *
   * Optional in same-origin proxy mode, where the backend resolves the site by hostname.
   *
...
export type SurgentaTracker = {
  track: {
     ...
     *
     * @example ```
     * surgenta.track('signup-button', { name: 'newsletter', id: 123 });
     * ```
     */
     ...
     *
     * @example ```
     * surgenta.track({ website: 'e676c9b4-11e4-4ef1-a4d7-87001773e9f2', url: '/home', title: 'Home page' });
     * ```
     */
     ...
     *
     * @example ```
     * surgenta.track((props) => ({ ...props, url: path }));
     * ```
     */
