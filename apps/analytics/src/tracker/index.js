;((window) => {
  ...
  const eventRegex = /data-surgenta-event-([\w-_]+)/
  const eventNameAttribute = `${_data}surgenta-event`
  ...
  const trackingDisabled = () =>
    disabled ||
    localStorage?.getItem('surgenta.disabled') ||
    ...
    try {
      const res = await fetch(endpoint, {
        ...
        headers: {
          ...
          ...(typeof cache !== 'undefined' && { 'x-surgenta-cache': cache }),
        },
        credentials,
      })
