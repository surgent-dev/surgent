...
function resolveRequestHostname(
  request: Request,
  payloadHostname: string | undefined,
): string | null {
  const headerHostname =
    request.headers.get('x-surgent-hostname') ||
    request.headers.get('x-forwarded-host') ||
    request.headers.get('x-original-host')

  return normalizeHostname(headerHostname) || normalizeHostname(payloadHostname)
}
  ...
  payload: z
    ...
    .refine(
      (data) => {
        ...
        return (
          count === 1 ||
          (count === 0 && typeof data.hostname === 'string' && data.hostname.length > 0)
        )
      },
      {
        message: 'Website, link, pixel, or hostname is required',
        path: ['website'],
      },
    ),
})
...
export async function POST(request: Request) {
  try {
    ...
    const hostname = resolveRequestHostname(request, payloadHostname)

    // Cache check
    let cache: Cache | null = null
    const cacheHeader = request.headers.get('x-surgenta-cache')
    ...
    let websiteId = payloadWebsiteId || cache?.websiteId

    if (!websiteId && !pixelId && !linkId) {
      if (!hostname) {
        return badRequest({ message: 'Hostname is required.' })
      }

      const website = await fetchWebsiteByHostname(hostname)
    ...
    if (payloadWebsiteId && !cache?.websiteId) {
      const website = await fetchWebsite(payloadWebsiteId)
    ...
    if (!sourceId) {
      return badRequest({ message: 'Unable to resolve analytics source.' })
    }
