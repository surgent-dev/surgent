...
- **Responsibilities**:
  ...
  - Payment processing via Whop integration.
...
- **Worker to Whop**: Payment processing, checkout configurations, and webhook handling.
...
1. **Subscription Management**: `@apps/web` interacts with `@apps/worker` to manage user subscriptions and payments via Whop.
2. **Event Processing**: Whop webhooks are received by `@apps/worker` and processed to update user status.
...
- `@apps/worker` validates session cookies via Better Auth.
